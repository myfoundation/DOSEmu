...
*/
// =======================================================================
// COPYRIGHT (c) 2019, 2025 MASTER MENTOR, MIT LICENSE
...
VERSION 1.0.4    23:34 28.07.2025
VERSION 1.0.5    10:48 29.07.2025
VERSION 1.0.10   17:55 31.07.2025
VERSION 1.0.15   00:35 06.09.2025
VERSION 1.0.16   12:38 07.09.2025
*/
...
//------------------------------------------
// SAMPLE HOW DO DIRECT VIDEO MEMORY WORK WITH TRUE COLOR_MODE
//------------------------------------------
void sample_direct_video_memory_work_with_true_color_mode()
{
	// 1
	...
	// USE THIS INSTEAD
	//------------------------------------------
	int screen_cx = 0, screen_cy = 0, pixel_bits = 0;
	_videomode_info(&screen_cx, &screen_cy, &pixel_bits);

	// ALLOC MEM BUFFER
	int mem_len = screen_cx*screen_cy*pixel_bits/8;
	BYTE*	mem = (BYTE*)malloc(mem_len);
	memset(mem, 0, mem_len);

	int pixel_x = 2, pixel_y = 2;
	int mem_offset = (pixel_y*screen_cx+pixel_x)*pixel_bits/8;

	memRGB(mem, mem_offset, 255,0,0);

	// MEM BUFFER -> SCREEN MEM
	_screen_lock(true);
	memmove(MEMORY_0xA0000000, mem, mem_len);
	_screen_lock(false);
...
void _videomode_info(int* screen_cx = NULL, int* screen_cy = NULL, int* pixel_bits = NULL);
void _screen_lock(BOOL is_lock); // LOCK / UNLOCK VIDEOMEM FOR MULTITHREADING
...
inline int colorRGB(BYTE R, BYTE G, BYTE B) { return int(R) | (int(G)<<8) | (int(B)<<16); }

inline void memRGB(BYTE* mem, int mem_offset, BYTE R, BYTE G, BYTE B)
{
	// MUST BE NOT RGB, BUT BGR
	mem[mem_offset+0] = B;		// B
	mem[mem_offset+1] = G;		// G
	mem[mem_offset+2] = R;		// R
}
...
struct BasicGraphRoutines
{
	...
	void init(int cx, int cy, BYTE* _VgaMem, int pixelbits = 8)
	{
		m_cx = cx;
		m_cy = cy;
		VgaMem_size = cx*cy;
...
*/
	...
	BYTE* XY_to_offset(int x,int y)
	{
		...
		BYTE* cell = VgaMem+(m_cx*y+x)*m_pixelbits/8;
	...
	*/
	void DrawPixel(int x,int y,int colour)
	{
		...
		if(m_pixelbits == 8) *cell= colour & 0xFF;
		else if(m_pixelbits == 24)
		{
			// RGB -> BGR
			cell[0] = (colour >> 16) & 0xFF;
			cell[1] = (colour >> 8) & 0xFF;
			cell[2] = colour & 0xFF;
		}
	}
	...
	THIS SAFE FUNCION FOR NEGATIV OR HIGH OFFSETS VALUES
	*/
	int GrabPixel(int x,int y)
	{
		BYTE* cell = XY_to_offset(x, y);
		if(cell == (BYTE*)-1) return 0;

		if(m_pixelbits == 8) return(*cell);
		else if(m_pixelbits == 24)
		{
			return colorRGB(cell[2],cell[1],cell[0]);
		}
	}
	...
	void DrawLine(int l,int t,int r,int b,int colour)
	{
	...
	FILLED RECTANGLE
	*/
	void DrawBar(int l,int t,int r,int b,int colour)
	{
		if(m_pixelbits == 8)
		{
			...
			for (;t<b;t++)
				memset(TVgaMem+m_cx*t,colour & 0xFF,width);
		}
		else if(m_pixelbits == 24)
		{
			// TODO
	...
	*/
	void DrawBox(int l,int t,int r,int b,int colour)
	{
		if(m_pixelbits == 8)
		{
			...
			BYTE* TVgaMem=VgaMem+l;
			memset(TVgaMem+m_cx*t,colour & 0xFF,width+1),t+=1;
			for (;t<b;t++)
				*(TVgaMem+m_cx*t)=colour,*(TVgaMem+m_cx*t+width)=colour;
			memset(TVgaMem+m_cx*(b-1),colour,width+1);
		}
		else if(m_pixelbits == 24)
		{
			// TODO
