...
struct DIBSurface
{
	...
	BYTE*  new_surface(int cx, int cy, int biBitCount = VALUE_NOT_DEFINED)
	{
		...
		return new_surface_raw_bytes_buffer((cx * cy) * biBitCount/8);
	}
...
struct VGAScreen : public CLockObject
{
	...
	void _setvideomode(short mode)
	{
		{ // LOCK STAT
			...
			switch(mode)
			{
				...
				case _ZRESTRUECOLOR:
					m_pixelbits = 24;
					m_screen->new_surface(m_cx, m_cy, 24);
				break;
				default:
					m_pixelbits = 8;
	...
	{
		AUTOLOCKTHIS;
		m_BasicGraphRoutines->init(m_cx, m_cy, GetVgaMem(), m_pixelbits);
	}
...
{
	// PALETTE, USED ONLY IN INDEXED 256COLORS MODES
...
void _videomode_info(int* screen_cx, int* screen_cy, int* pixel_bits)
{
	AUTOLOCK(*vgaScreenPrimary);

	if(screen_cx) *screen_cx = vgaScreenPrimary->m_cx;
	if(screen_cy) *screen_cy = vgaScreenPrimary->m_cy;
	if(pixel_bits) *pixel_bits = vgaScreenPrimary->m_pixelbits;
}
...
void _screen_lock(BOOL is_lock)
{
	if(is_lock) vgaScreenPrimary->Lock();
	else vgaScreenPrimary->Unlock();
}
...
#define WM_KEYUP_CONSOLE (WM_USER + 2)
#define WM_KEYDOWN_CONSOLE (WM_USER + 3)
...
LRESULT CALLBACK ScereenWndProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	switch (message)
	{
		case WM_KEYUP:
		case WM_KEYDOWN:
		case WM_KEYUP_CONSOLE:
		case WM_KEYDOWN_CONSOLE:
		{ // LOCK START
			AUTOLOCK(emuKernel);

			if(wparam >= WIN_VK_KEYS_TOTAL) return 0;
	...
	switch (message)
	{
		...
		case WM_KEYDOWN_CONSOLE:
		{
			...
			if(chr)
			{
				...
				DWORD dwTmp = 0;
				if(message != WM_KEYDOWN_CONSOLE) WriteConsoleInput(GetStdHandle(STD_INPUT_HANDLE), ir, 1, &dwTmp);
			}
			//------------------------------------------
...
*/
ULONG WINAPI ConsoleInputThread(LPVOID pParam)
{
    ...
    while (true)
	{
        ...
        if (ReadConsoleInput(hConsole, &inputRecord, 1, &eventsRead))
		{
			if (inputRecord.EventType == KEY_EVENT)
			{
				...
				{
					ScereenWndProc(NULL, WM_KEYDOWN_CONSOLE, inputRecord.Event.KeyEvent.wVirtualKeyCode, 0);
				}
				else
				{
					ScereenWndProc(NULL, WM_KEYUP_CONSOLE, inputRecord.Event.KeyEvent.wVirtualKeyCode, 0);
				}
			}
...
#define TIMER_8253_FREQ 1193180 // 1.19318 MHz

#define TIMER_8253_60HZ    0x4DAE // 60 hz
#define TIMER_8253_30HZ    0x965C // 30 hz
#define TIMER_8253_20HZ    0xE90B // 20 hz
#define TIMER_8253_18HZ    0xFFFF // 18.2 hz (the standard count)

#define TIMER_8253_40h  0x40  // counter 0
#define TIMER_8253_41h  0x41  // counter 1
#define TIMER_8253_42h  0x42  // counter 2
#define TIMER_8253_43h  0x43  // the 8253's control register

#define CW_SC1_1  0b10000000
#define CW_SC1_0  0b00000000
#define CW_SC0_1  0b01000000
#define CW_SC0_0  0b00000000
#define CW_RL1_1  0b00100000
#define CW_RL1_0  0b00000000
#define CW_RL0_1  0b00010000
#define CW_RL0_0  0b00000000
#define CW_M2_1   0b00001000
#define CW_M2_0   0b00000000
#define CW_M1_1   0b00000100
#define CW_M1_0   0b00000000
#define CW_M0_1   0b00000010
#define CW_M0_0   0b00000000
#define CW_BCD_1  0b00000001
#define CW_BCD_0  0b00000000

#define CW_SELECT_COUNTER0	(CW_SC1_0 | CW_SC0_0)
#define CW_SELECT_COUNTER1	(CW_SC1_0 | CW_SC0_1)
#define CW_SELECT_COUNTER2	(CW_SC1_1 | CW_SC0_0)
#define CW_READ_LOAD_COUNTER_LATCH 		(CW_RL1_0 | CW_RL0_0)
#define CW_READ_LOAD_BYTE_MOST			(CW_RL1_1 | CW_RL0_0)
#define CW_READ_LOAD_BYTE_LEAST			(CW_RL1_0 | CW_RL0_1)
#define CW_READ_LOAD_BYTE_LEAST_MOST	(CW_RL1_1 | CW_RL0_1)
#define CW_MODE_0						(CW_M2_0 | CW_M1_0 | CW_M0_0)
#define CW_MODE_1						(CW_M2_0 | CW_M1_0 | CW_M0_1)
#define CW_MODE_2_0						(CW_M2_0 | CW_M1_1 | CW_M0_0)
#define CW_MODE_2_1						(CW_M2_1 | CW_M1_1 | CW_M0_0)
#define CW_MODE_3						(CW_M2_0 | CW_M1_1 | CW_M0_1)
#define CW_MODE_4						(CW_M2_1 | CW_M1_0 | CW_M0_0)
#define CW_MODE_5						(CW_M2_1 | CW_M1_0 | CW_M0_1)
#define CW_BCD_16BIT  					CW_BCD_0
#define CW_BCD_4DECADES  				CW_BCD_1

#define CW_SET_TIMER_0	(CW_READ_LOAD_BYTE_LEAST_MOST | CW_MODE_2_0 | CW_BCD_16BIT)
#define CW_SET_TIMER_1	(CW_READ_LOAD_BYTE_LEAST_MOST | CW_MODE_2_1 | CW_BCD_16BIT)

struct TIMER_8253_STRUCT
{
	long freq_counter0;
	char control_word;
	int rw_ndx;
...
void _outp(int port, int value)
{
	...
	case PALETTE_DATA__:
		{
			...
			if(scr->m_VGA_RENDER_state & RENDER_CORRECT_PALETTE)
			{
				value = ((unsigned int)value) << 2;
			}

			scr->m_screen->m_surfacePalette[scr->INOUT_palette_offset*3 + scr->INOUT_palette_write_count] = value;
			scr->INOUT_palette_write_count++;
			if(scr->INOUT_palette_write_count == 3)
			{
				PaletteRAW2RGB(scr->m_screen->m_surfaceBI->bmiColors,
					scr->m_screen->m_surfacePalette,
					scr->INOUT_palette_offset, 1);

				if(!(scr->m_VGA_RENDER_state &
	...
	// TIMER WORK
	//------------------------------------------
	case TIMER_8253_43h:  // the 8253's control register
		{
			AUTOLOCK(emuKernel);
			TIMER_8253.control_word = value;
			TIMER_8253.rw_ndx = 0;
		}
		break;
	case TIMER_8253_40h:  // counter 0
		if((TIMER_8253.control_word == CW_SET_TIMER_0) ||
			(TIMER_8253.control_word == CW_SET_TIMER_1))
		{
			AUTOLOCK(emuKernel);

			switch(TIMER_8253.rw_ndx)
			{
				case 0:
					TIMER_8253.r_lo = value;
					TIMER_8253.rw_ndx++;
				break;
				case 1:
					TIMER_8253.r_hi = value;
					TIMER_8253.freq_counter0 = (TIMER_8253.r_hi << 8) | TIMER_8253.r_lo;
				break;
			}
		}
		break;
	...
	case PALETTE_DATA__:
		{
			...
			ret = scr->m_screen->m_surfacePalette[scr->INOUT_palette_offset*3 + scr->INOUT_palette_write_count];
			scr->INOUT_palette_write_count++;
...
ULONG WINAPI TimerThread(LPVOID pParam)
{
	...
	while(!emuKernel.IsStop(freq_counter0))
	{
		Sleep(freq_counter0); // MEMORY_0x0000046C TIMER RATE

		{ // LOCK START
			AUTOLOCK(emuKernel);
			DOS_55_MS_TIMER++;

			freq_counter0 = 1000.0 / (double(TIMER_8253_FREQ) / double(TIMER_8253.freq_counter0));
		} // LOCK END
	...
	case MOUSE_INT__:
		{
			...
			case MOUSE_BUTT_POS__:
			{
				...
				if(scr->m_screenHWND && GetCursorPos(&p))
				{
					//cursor position now in p.x and p.y
					if (ScreenToClient(scr->m_screenHWND, &p))
					{
						//p.x and p.y are now relative to hwnd's client area
						if(scr->m_VGA_RENDER_state & RENDER_SCALE_VGA_SCREEN)
						{
							p.x = p.x / scr->m_scale;
							p.y = p.y / scr->m_scale;
						}
				...
				if(p.x < 0 || p.x > scr->m_cx) p.x = 0;
				if(p.y < 0 || p.y > scr->m_cy) p.y = 0;
...
void _window_cmd(int cmd)
{
	HWND screenHWND = NULL;
	HWND hWndConsole = GetConsoleWindow();

	SAFE_OBJECT(*vgaScreenPrimary, screenHWND = vgaScreenPrimary->m_screenHWND);

	switch(cmd)
	{
	case WINDOW_VGA_HIDE:
		ShowWindow(screenHWND, SW_HIDE);
		break;
	case WINDOW_VGA_SHOW:
		ShowWindow(screenHWND, SW_SHOW);
		break;
	case WINDOW_CONSOLE_HIDE:
		ShowWindow(hWndConsole, SW_HIDE);
		break;
	case WINDOW_CONSOLE_SHOW:
		ShowWindow(hWndConsole, SW_SHOW);
		break;
	case WINDOW_VGA_ACTIVE:
		SetForegroundWindow(screenHWND);
		break;
	case WINDOW_CONSOLE_ACTIVE:
...
int main(int argc, char* argv[])
{
	...
	// INIT TIMER DEVICE
	memset(&TIMER_8253, 0, sizeof(TIMER_8253));
	TIMER_8253.freq_counter0 = TIMER_8253_18HZ;
	...
	emuKernel.m_threads[2] = Thread((void*)ConsoleInputThread, NULL, FALSE);
